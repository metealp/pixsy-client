...
export default view(() => {
    ...
    const [modalPhoto, setModalPhoto] = useState(emptyModalPhoto)
    ...
    const items = appStore.isCollectionLoading
        ? <LoadingSpinner />
        : (topicPhotos &&
            topicPhotos.map((photo) => {
                const thumbnailUrl = createThumbnail(photo.url)
                return (
                    <Image
                        preview={false}
                        onClick={() => showModal(photo)}
                        rootClassName='topicImage'
                        lazy
                        width={200}
                        height={200}
                        src={thumbnailUrl}
    ...
    return (
        <Layout sider={<SideNav />} main={
                <>
                    <Grid items={items} isLoading={appStore.isCollectionLoading} />
                    <Modal
                        title={`Photo #${modalPhoto.id}`}
                        open={isModalOpen}
                        footer={[]}
                        className='topicPhotoModal'
                        onOk={hideModal}
                        onCancel={hideModal}
                        destroyOnClose={true}
                    >
